    ...
    public function newCarImageFormPost(EleganceImageFormRequest $request)
    {
        ...
        /*
            ...
            if($request->hasFile('file'))
            {
                ...
                // Laravel may not have a way to extract just the filename without extension. You can do this using PHP:
                // Get just filename
                ...
                // Filename to store
                $filenameToStore = $filename . '_' . time() . '.' .  $extension; // E.g. "my_pix_545676788.jpg"

                // Upload image
                $path = $request->file('file')->storeAs('public/VehiclesInStock/images/', $filenameToStore);
            }
            else {
                    $filenameToStore = 'defaultImage.jpg';
            }

            NOTE: The 'public/VehiclesInStock/images/' will be created in 'storage/app' not 'public/'. Consequently, 'public/' will not have the files stored there.

            So we should run the following command:
            php artisan storage:link

            NOTE: This will link the 'storage' and 'public' directories in the root level. Now, the following 'storage' folder be created in 'public' folder. ''



         */

        $image = $request->file('file'); //This will have filename and its extension
        ...
        /*
            In Laravel, instead of using the following snippet:
            $image->move($destinationPath, $hashedName);

            You may use:
            $path = $image->storeAs('public/VehiclesInStock/images/', $hashedName);
...
*/
    ...
    public function editCarImageFormPost(EleganceImageFormEditRequest $request, $ImageId)
    {
        ...
        $data = array(
            ...
            'disk_image_filename' => $hashedName,

            /*
            ### Brad Traversy's way ###
            'disk_image_filename' => $filenameToStore,
            */
    ...
    public function imageDelete($ImageId, $PostId)
    {
        ...
        if($post->profilePhoto != 'defaultImage.jpg')
        {
            Storage::delete('public/VehiclesInStock/images/' . $post->profilePhoto)
        }
